...
```
For face detection and ID-embedding extraction, download the [antelopev2](https://github.com/deepinsight/insightface/tree/master/python-package) package and place the checkpoints under `models/antelopev2`. We use an ArcFace trained on WebFace42M. Download it from [here], delete `glintr100.onnx` and replace it with `model.onnx` in `models/antelopev2`. The `models` folder structure should finally be:
