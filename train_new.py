...
# def save_pretrained_new(self, path):
...
#     if self.model_cls==UNet2DConditionModel:
#         # UNet2DConditionModel 的保存逻辑
#         model = self.model_cls.from_config(self.model_config)

#     elif self.model_cls==CLIPTextModel:
#         model = self.model_cls._from_config(self.model_config)

#     else:
#         raise ValueError("Unsupported model type for save_pretrained_model.")
...
def main():
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
            ...
            if accelerator.is_main_process:
                ...
                for i, model in enumerate(models):
                    ...
                    if isinstance(model,CLIPTextModel):
                        newlogger=logging.getLogger('transformers.modeling_utils').setLevel(logging.INFO)
        ...
        def load_model_hook(models, input_dir):
            if args.use_ema:
                ...
                # load_model_text_encoder = EMAModel.from_pretrained(
                #     os.path.join(input_dir, "text_encoder_ema"), CLIPTextModel, foreach=args.foreach_ema
                # )
                ema_unet.load_state_dict(load_model.state_dict())
                #ema_text_encoder.load_state_dict(load_model_text_encoder.state_dict())
                if args.offload_ema:
                    ema_unet.pin_memory()
                    #ema_text_encoder.pin_memory()
                else:
                    ema_unet.to(accelerator.device)
                    #ema_text_encoder.to(accelerator.device)

                del load_model
                #del load_model_text_encoder
    ...
    dataset=bulid_dataset('train',train_transforms,args)
