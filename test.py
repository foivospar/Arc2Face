...
def main():
    ...
    def set_seed(seed: int):
        ...
        torch.backends.cudnn.deterministic = True  # 确保确定性的行为
        torch.backends.cudnn.benchmark = False  # 禁用与硬件相关的优化

    def worker_init_fn(worker_id):
        """设置每个 worker 的随机种子"""
        seed = torch.initial_seed() % 2**32  # 每个worker唯一的随机种子
        np.random.seed(seed)
        random.seed(seed)
